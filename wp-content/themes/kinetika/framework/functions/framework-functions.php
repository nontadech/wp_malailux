...
function mtheme_add_og_metatags() {
	...
	if ( is_singular() && $og_meta_added==false ) {
		...
		echo sprintf( '<meta property="og:site_name" content="%s"/>', get_bloginfo('name') );
		echo sprintf( '<meta property="og:description" content="%s"/>', mtheme_shortentext( esc_html( $post->post_excerpt ), 50 ) );
...
function mtheme_extract_googlefont_data( $got_font ) {
	if ( ! MTHEME_BUILDMODE ) {
		if ($got_font) {
			...
			$font_url = 'http://fonts.googleapis.com/css?family='.$font_name . ':' . $font_variants . $font_subsets;
