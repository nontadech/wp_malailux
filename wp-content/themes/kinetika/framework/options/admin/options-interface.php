...
function optionsframework_tabs() {
	...
	foreach ($options as $value) {
		...
		if ($value['type'] == "heading") {
			...
			$menu .= '<a id="'.  esc_attr( $jquery_click_hook ) . '-tab" class="nav-tab '.esc_attr($subheading_class).'" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#'.  $jquery_click_hook ) . '">' . esc_html( $value['name'] ) . '</a>';
		}
	}
...
function optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		case 'dragdrop_sorter':
			...
			if ( is_array( $val ) ) {
				foreach ($value['options'] as $key => $option) {
					...
					$sortoutput .=  '<img src="'. esc_url( $directory_uri . $image_page . 'minus.png' ) . '" class="action" title="Delete"/>';
			...
			$sortoutput .=  "</div>";
			$sortoutput .=  "</div>";
			$sortoutput .=  '<div class="theme_arrow"><img src="'. esc_url( $directory_uri . $image_page . 'arrow_drag.png' ) . '" /></div>';
			...
			$sortoutput .=  "</div>";
			$sortoutput .=  "</div>";
			$sortoutput .= '<div id="values" data-key="'.esc_attr($option_name).'"></div>';
			$sortoutput .=  "</div>";
		...
		case 'exporttextarea':
			...
			$output .= '<textarea readonly id="' . esc_attr( $value['id'] ) . '" class="of-input opt-textbox-'.$textarea_class.'" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" cols="'. esc_attr( $cols ) . '" rows="8">' . esc_textarea($val) . '</textarea>';
		...
		case 'importtextarea':
			...
			$output .= '<textarea id="' . esc_attr( $value['id'] ) . '" class="of-input opt-textbox-'.$textarea_class.'" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" cols="'. esc_attr( $cols ) . '" rows="8">' . esc_textarea($val) . '</textarea>';
			$output .= '<div><input type="submit" class="button button-secondary button-large" name="import" value="Import Options" /></div>';
		...
		case 'textarea':
			...
			$output .= '<textarea '.$readonly_mode.' id="' . esc_attr( $value['id'] ) . '" class="of-input opt-textbox-'.$textarea_class.'" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" cols="'. esc_attr( $cols ) . '" rows="8">' . esc_textarea($val) . '</textarea>';
		break;
