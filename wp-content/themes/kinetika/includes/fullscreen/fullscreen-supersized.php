...
<?php
	...
	foreach ( $filter_image_ids as $attachment_id) {
		...
		if ( !$static_msg_display ) {
			...
			if ($link_text) $slideshow_link ='<div class="slideshow_content_link '.$slideshow_no_description_no_title.'"><a class="supersized-button" href="'.esc_url($link_url).'">'. esc_attr($link_text) .'</a></div>';
		...
		$slideshow_display_code = '<div class="slideshow-content-wrap">' .$slideshow_title . $slideshow_caption . $slideshow_link . '</div>';
