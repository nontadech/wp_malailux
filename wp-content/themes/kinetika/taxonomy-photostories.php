...
$portfolio_layout=of_get_option('gallery_achivelisting');
...
<?php
$format=of_get_option('gallery_archive_format');
if ( $format<>"landscape" && $format<>"square" && $format<>"portrait" && $format<>"masonary" ) {
	$format="landscape";
}
echo do_shortcode('[gallerygrid displaycategory="false" worktype_slugs="'.$worktype.'" format="'.$format.'" type="default" limit="-1" pagination="true" columns="'.$columns.'"]');
?>
</div>
<?php get_footer(); ?>